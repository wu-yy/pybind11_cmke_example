import re
import os
from leetcode.utils import LOG, run_cmd
...
def compile_src(src, h , basename):
    ...
    # if not h.endswith("init.h"): return
    class_ranges = None
    class_name = None
    class_info = None
    submodule_name = None
    ...
    # include_name = h[2:] # remove "src/" prefix
    include_name = h.split('src/')[-1]
    code = []
    class_defs_code = []
    class_getsets_code = []
    ...
    for name in def_targets:
        ...
        # LOG.i(slot_name, " ", func_head, " ", func_cast)

        arr_func_return = []
        doc_all = ""
        decs = "The function declarations are:\n"
        # LOG.i(arr_has_return)
        # arr_func_call like: 'set_seed(arg0)'
        # LOG.i(arr_func_call)
        ...
        # arr_func_return like: ['return GET_PY_NONE((set_seed(arg0)))']
        # LOG.i(arr_func_return)
        # generate error msg when not a valid call
        error_log_code = generate_error_code_from_func_header(func_head, target_scope_name, name,
...
def compile(cache_path, project_path):
    headers1 = run_cmd('find -L src | grep ".h$"', project_path).splitlines()
    headers2 = run_cmd('find gen/ | grep ".h$"', cache_path).splitlines()
    headers = [ os.path.join(project_path, h) for h in headers1 ] + \
    ...
    """
    fname = os.path.join(cache_path, "gen", "pyjt_all.cpp")
